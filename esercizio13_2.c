...
struct mailbox_t
{
    ...
    pthread_cond_t cond, cond_riceventi, synchro_R;

    msg array[DIM_BUFFER];
    int testa_array;
    int coda_array, coda1, coda2;
    int stato_di_lettura1[DIM_BUFFER];
    int stato_di_lettura2[DIM_BUFFER]; //se msg i-esimo vale 2 allora potrà essere cancellato
    int spazio_occupato;
...
void init_mailbox(struct mailbox_t *m)
{
    ...
    pthread_cond_init(&m->synchro_R, &cattr);
    ...
    m->spazio_occupato = m->testa_array = m->coda_array = m->coda1 = m->coda2 = 0;
}
...
void receive1(struct mailbox_t *m)
{
    ...
    if (m->stato_di_lettura1[m->coda1] == 0)
    {
        ...
        pthread_cond_signal(&m->synchro_R);
    }

    if (m->stato_di_lettura1[m->coda_array] == 1 && m->stato_di_lettura2[m->coda_array] == 1)
    {
        m->spazio_occupato--;
...
void receive2(struct mailbox_t *m)
{
    ...
    while(m->stato_di_lettura1[m->coda2] == 0){
        pthread_cond_wait(&m->synchro_R, &m->mutex);
    }
